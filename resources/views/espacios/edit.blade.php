...
<div class="max-w-4xl mx-auto">
    <div class="flex items-center gap-4 mb-10 border-b border-gray-100 pb-8">
        ...
        <div>
            <h1 class="text-4xl font-black text-gray-900 tracking-tighter">Editar Espacio</h1>
            <p class="text-gray-500 mt-1 italic">Modifica los detalles del espacio logístico seleccionado.</p>
        </div>
    </div>

    <div class="bg-white p-10 rounded-3xl shadow-2xl border border-gray-100">
        <form action="{{ route('espacios.update', $espacio->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @method('PUT') 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nombre" required value="{{ old('nombre', $espacio->nombre) }}" class="w-full bg-gray-50 border-none focus:ring-2 focus:ring-blue-500 rounded-2xl p-4 text-gray-800 font-medium transition-all">
                </div>
                ...
                <div>
                    ...
                    <select name="torre_id" required class="w-full bg-gray-50 border-none focus:ring-2 focus:ring-blue-500 rounded-2xl p-4 text-gray-800 font-medium appearance-none">
                        @foreach($torres as $torre)
                            <option value="{{ $torre->id }}" {{ $espacio->torre_id == $torre->id ? 'selected' : '' }}>
                                {{ $torre->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="capacidad_personas" min="1" required value="{{ old('capacidad_personas', $espacio->capacidad_personas) }}" class="w-full bg-gray-50 border-none focus:ring-2 focus:ring-blue-500 rounded-2xl p-4 text-gray-800 font-medium transition-all">
                </div>
            </div>

            <div>
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Descripción (Opcional)</label>
                <textarea name="descripcion" rows="3" class="w-full bg-gray-50 border-none focus:ring-2 focus:ring-blue-500 rounded-2xl p-4 text-gray-800 font-medium transition-all">{{ old('descripcion', $espacio->descripcion) }}</textarea>
            </div>
            ...
            <div>
                ...
                @if($espacio->imagen_url)
                    <div class="flex items-center gap-4 mb-4 bg-gray-50 p-4 rounded-2xl border border-gray-100 w-fit">
                        <img src="{{ asset($espacio->imagen_url) }}" class="w-20 h-20 object-cover rounded-xl shadow-md">
                        <div>
                            <p class="text-xs font-bold text-gray-500">Imagen actual en el servidor</p>
                            <p class="text-[10px] text-gray-400">Si subes una nueva, esta se reemplazará automáticamente.</p>
                        </div>
                    </div>
                @endif
